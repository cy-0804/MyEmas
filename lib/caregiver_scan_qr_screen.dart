  ...
  Future<void> _onDetect(BarcodeCapture capture) async {
    ...
    try {
      ...
      final userRow = await db
          .from('users')
          .select('user_id, fullname, role_id')
          .eq('user_id', code)
          .maybeSingle();
      if (userRow == null) throw Exception('Invalid QR code – user not found');
      if (userRow['role_id'] != 'elderly') {
        throw Exception('This QR belongs to a non-elderly account');
      }

      // Make sure elderly record exists
      final elderlyRow = await db
          .from('elderly')
          .select('user_id')
          .eq('user_id', code)
          .maybeSingle();
      ...
      if (existing != null) {
        setState(() {
          _status =
              'Already linked to ${userRow['fullname'] ?? 'this patient'}';
          _linking = false;
        });
      ...
      setState(() {
        _status =
            '✅ Successfully linked to ${userRow['fullname'] ?? 'patient'}!';
        _linking = false;
      });
      await Future.delayed(const Duration(seconds: 2));
      if (mounted) Navigator.pop(context, true);
              ...
              child: Column(
                children: [
                  ...
                  if (_status == null && !_linking) ...[
                    ...
                    Text(
                      'The QR code can be found in the patient\'s MyEmas app',
                      textAlign: TextAlign.center,
                      style: TextStyle(
...
class _ScanimationDotState extends State<_ScanimationDot>
    ...
    return AnimatedBuilder(
      ...
      builder: (_, _) => Opacity(
        ...
        child: Container(
          ...
          child: Center(
            child: Icon(Icons.add, color: Color(0xFF51A77B), size: 14),
          ),
        ),
      ),
    );
